...
.DESCRIPTION
    Connects to Microsoft Graph first, then Exchange Online, to avoid a dependency
    assembly conflict between Microsoft.Graph.Authentication and ExchangeOnlineManagement.

    Existing Graph and Exchange Online connections are reused. Only connections created
    by this script are disconnected at completion.
...
.PARAMETER SkipConfirmation
    Suppress confirmation prompts for changes.
...
.EXAMPLE
    .\Remove-UnlicensedMailboxHolds.ps1 -WhatIf -DisableAllHolds -ReportTo rykov@ottawa.ca

.EXAMPLE
    .\Remove-UnlicensedMailboxHolds.ps1 -DisableAllHolds -SkipConfirmation -ReportTo rykov@ottawa.ca
...
function Assert-CommandAvailable {
    ...
    if (-not (Get-Command -Name $CommandName -ErrorAction SilentlyContinue)) {
        throw "Required command '$CommandName' is unavailable after loading '$ModuleName'."
    }
}
...
have the required User.Read.All delegated scope. The script will not replace a
pre-existing Graph connection. Start a new PowerShell session or reconnect Graph with
User.Read.All before running this script.
...
function Get-InPlaceHoldsForMailbox {
    param(
        ...
        [Parameter(Mandatory)][object[]]$AllInPlaceHolds
    )
...
function Remove-MailboxFromInPlaceHold {
    ...
    $updatedSources = @($sourceMailboxes | Where-Object { $_.ToString() -ine $UserPrincipalName })
    ...
    if ($WhatIfPreference) {
        Write-Host "[WhatIf] Would $operation." -ForegroundColor DarkYellow
        return [PSCustomObject]@{
            HoldName = $Hold.Name
    ...
    if (-not $PSCmdlet.ShouldProcess($UserPrincipalName, $operation)) {
        return [PSCustomObject]@{
            HoldName = $Hold.Name
            HoldStatus = 'Skipped-Confirmation'
        }
    }
...
"@
    ...
    $emailSubject = if ($WhatIfPreference) { "[WhatIf] $Subject" } else { $Subject }

    Send-MailMessage -SmtpServer 'appsmtp.ottawa.ca' -From $From -To $To `
        -Subject $emailSubject -Body $body -Attachments $CsvPath `
...
if ($ReportTo -and $ReportTo.Count -gt 0) {
    $sendReport = $true
}
else {
    $sendReport = $false
...
try {
    ...
    # Graph must be loaded and connected before Exchange Online. This prevents
    # EXO from loading conflicting MSAL/IdentityModel assemblies first.
    ...
    $graphState = Get-GraphConnectionState
    $graphConnectedByScript = Connect-GraphService -State $graphState

    # EXO is deliberately loaded and connected only after Graph authentication.
    ...
    $exchangeState = Get-ExchangeConnectionState
    $exchangeConnectedByScript = Connect-ExchangeService -State $exchangeState

    if (($DisableLitigationHold -or $DisableInPlaceHold) -and -not $WhatIfPreference -and -not $SkipConfirmation) {
        if (-not $PSCmdlet.ShouldContinue(
                'This will modify holds on unlicensed mailboxes in Exchange Online. Continue?',
                'Confirm Hold Remediation')) {
            Write-Host '[!] Operation cancelled by user.' -ForegroundColor Yellow
            return
        }
    }

    Write-Host '[*] Retrieving mailboxes with Litigation Hold enabled...' -ForegroundColor Cyan
    ...
    foreach ($upn in $upnList) {
        $counter++
        $percent = if ($upnList.Count -gt 0) { [math]::Round(($counter / $upnList.Count) * 100) } else { 100 }
        Write-Progress -Id 1 -Activity 'Checking Microsoft 365 license status' `
            -Status "[$counter / $($upnList.Count)] $upn" -PercentComplete $percent
        ...
        try {
            ...
            if (@($graphUser.AssignedLicenses).Count -eq 0) {
    ...
    foreach ($upn in $workList) {
        $counter++
        $percent = if ($workList.Count -gt 0) { [math]::Round(($counter / $workList.Count) * 100) } else { 100 }
        Write-Progress -Id 2 -Activity 'Processing unlicensed mailboxes' `
            -Status "[$counter / $($workList.Count)] $upn" -PercentComplete $percent

        $graphUser = $graphUserCache[$upn]
        $litigationRow = $litigationHoldMailboxes |
            Where-Object { $_.UserPrincipalName -ieq $upn } |
            Select-Object -First 1
        $matchingInPlaceHolds = @(Get-InPlaceHoldsForMailbox -UserPrincipalName $upn -AllInPlaceHolds $allInPlaceHolds)
        ...
        if ($hasLitigationHold) {
            ...
            elseif ($WhatIfPreference) {
                Write-Host "[WhatIf] Would disable Litigation Hold on '$upn'." -ForegroundColor DarkYellow
            ...
            elseif ($PSCmdlet.ShouldProcess($upn, 'Disable Litigation Hold')) {
                ...
                $litigationAction = 'LitigationHoldDisabled'
                $litigationStatus = 'Skipped-Confirmation'
            }
        }
    ...
    $mode = switch ($true) {
        ($WhatIfPreference -and $DisableLitigationHold -and $DisableInPlaceHold) { 'WhatIf — all holds would be remediated'; break }
        $WhatIfPreference { 'WhatIf — no changes made'; break }
        ($DisableLitigationHold -and $DisableInPlaceHold) { 'Litigation and In-Place Hold remediation'; break }
        $DisableLitigationHold { 'Litigation Hold remediation only'; break }
        $DisableInPlaceHold { 'In-Place Hold remediation only'; break }
        default { 'Report only' }
    }
    ...
    if ($sendReport) {
        try {
            Send-HoldReport -CsvPath $effectiveCsv -To $ReportTo -From $ReportFrom `
                -Subject $ReportSubject -Mode $mode -ResultCount $results.Count
    ...
    Disconnect-ScriptServices -GraphConnectedByScript $graphConnectedByScript `
